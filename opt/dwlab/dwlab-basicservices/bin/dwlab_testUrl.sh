...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --yamlFile)
        if [[ -n "$2" && ! "$2" =~ ^- ]]; then
            yamlFile="$2"
            [[ "$verbose" == true ]] && echo "DW-Lab: yamlFile=$yamlFile"
            shift 2
        else
    ...
    --port)
        if [[ -n "$2" && ! "$2" =~ ^- ]]; then
            port=":"$2
            [[ "$verbose" == true ]] && echo "DW-Lab: port=$port"
            shift 2
        else
    ...
    --url_extension)
        if [[ -n "$2" && ! "$2" =~ ^- ]]; then
            url_extension="$2"
            [[ "$verbose" == true ]] && echo "DW-Lab: url_extension=$url_extension"
            shift 2
        else
    ...
    -o|--output-file)
        if [[ -n "$2" && ! "$2" =~ ^- ]]; then
            outputFile="$2"
            [[ "$verbose" == true ]] && echo "DW-Lab: outputFile=$outputFile"
            shift 2
        else
...
yaml_helper="$helper_dir/dwlab_bash_yaml.sh"

[[ "$verbose" == true ]] && echo "DW-Lab: Read YAML file, containing runtime settings"
if [ ! -r "$yaml_helper" ]; then
  echo 'DW-Lab: *********************************************'
  echo "DW-Lab: $thisScript:"
  echo "DW-Lab: Cannot source file $yaml_helper"
...
then
  echo "DW-Lab: *********************************************"
  echo "DW-Lab: $0:"
  echo "DW-Lab: Please define the file $yamlFile"
  echo "DW-Lab: Aborting ... "
  echo "DW-Lab: *********************************************"
  exit 8
fi
